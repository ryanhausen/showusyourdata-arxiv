...
class ParsedFile:
    ...
    def parse_authors_from_soup(doc: bs4.BeautifulSoup) -> List[Author]:
        return list(
            map(
                Author.from_soup,
                filter(lambda x: x.attrs["role"] == "author", doc.find_all("creator")),
            )
        )
    ...
    def parse_text_from_soup(doc: bs4.BeautifulSoup) -> str:
        return list(map(ParsedFile.parse_text_from_section, doc.find_all("section")))
    ...
    def parse_text_from_section(section_tag: bs4.element.Tag) -> str:
        return dict(
            ...
            text=" ".join(
                list(map(lambda s: clean(s.text), section_tag.find_all("p")))
            ),
        )
...
def parse_file(file_path: str) -> Union[Dict[str, str], str]:
    ...
    return ParsedFile(
        ...
        text=[ParsedFile.parse_abstract_from_soup(doc)]
        + ParsedFile.parse_text_from_soup(doc),
    )
